...
class Bithumb():
    def __init__(self, order_currency):
        ...
        self._ccxt_bithumb = ccxt.bithumb({
            'apiKey': auth['api_key'],
            'secret': auth['secret'],
            'verbose': False
        })
    ...
    def get_orderbook(self):
        ...
        response = requests.get(
            f'https://api.bithumb.com/public/orderbook/{self.order_currency}_{payment_currency}')
    ...
    def trade_place_buy(self, units, price):
        return self._ccxt_bithumb.private_post_trade_place({
    ...
    def user_transactions(self, offset):
        response = self._ccxt_bithumb.private_post_info_user_transactions({
            'offset': offset,
            'count': 50,
            'order_currency': self.order_currency,
            'payment_currency': 'KRW'
    ...
    def private_post_info_orders(self):
        try:
            response = self._ccxt_bithumb.private_post_info_orders({
                'order_currency': self.order_currency,
                'payment_currency': 'KRW'
            })
        except:
    ...
    def trade_market_buy(self, units):
        response = self._ccxt_bithumb.private_post_trade_market_buy({
            'order_currency': self.order_currency,
            'payment_currency': 'KRW',
            'units': units
        })
    ...
    def trade_market_sell(self, units):
        response = self._ccxt_bithumb.private_post_trade_market_sell({
            'order_currency': self.order_currency,
            'payment_currency': 'KRW',
            'units': units
        })
    ...
    def get_candlestick_current_interval(self, interval):
        ...
        response = self._ccxt_bithumb.public_get_candlestick_currency_interval({
